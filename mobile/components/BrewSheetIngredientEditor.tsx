...
type Props = {
  initial?: { ingredientId: string; quantity: number; totalPrice?: number | null };
  ingredientOptions: SelectOption[];
  submitLabel: string;
  onSubmit: (input: { ingredientId: string; quantity: number; totalPrice: number | null }) => void;
  onCancel: () => void;
  isSubmitting?: boolean;
};

export function BrewSheetIngredientEditor({ initial, ingredientOptions, submitLabel, onSubmit, onCancel, isSubmitting }: Props) {
  const [ingredientId, setIngredientId] = useState<string | null>(initial?.ingredientId ?? null);
  const [quantity, setQuantity] = useState(initial?.quantity != null ? String(initial.quantity) : '');
  const [totalPrice, setTotalPrice] = useState(initial?.totalPrice != null ? String(initial.totalPrice) : '');

  const canSubmit = ingredientId && quantity.trim() && Number(quantity) > 0;

  function handleSubmit() {
    if (!canSubmit) return;
    onSubmit({ ingredientId, quantity: Number(quantity), totalPrice: totalPrice.trim() ? Number(totalPrice) : null });
  }
  ...
  return (
    <View style={styles.container}>
      <SelectField
        ...
        emptyHint="Najprv pridajte surovinu v záložke Sklad."
      ...
      <View style={styles.row}>
        ...
        <View style={styles.col}>
          ...
          <TextInput style={styles.input} keyboardType="numeric" value={totalPrice} onChangeText={setTotalPrice} placeholder="0" />
        </View>
      </View>
