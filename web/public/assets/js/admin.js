...
(function () {
  ...
  var DEFAULTS = {
    ...
    heroOverlayColor: '#0d2b33', radius: 14, buttonStyle: 'rounded'
  };
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ...
      '.pod-panel{position:fixed;top:0;right:0;bottom:0;width:360px;max-width:92vw;background:#1c1c1c;color:#e8e8e8;z-index:320;overflow-y:auto;transform:translateX(100%);transition:transform .25s ease;font:14px/1.5 system-ui,sans-serif}',
      ...
      '.pod-modal{position:fixed;inset:0;z-index:340;background:rgba(0,0,0,.55);display:flex;align-items:flex-start;justify-content:center;overflow-y:auto}',
      '.pod-modal__box{background:#fff;color:#222;width:min(920px,94vw);margin:5vh 0;border-radius:12px;overflow:hidden}',
      '.pod-modal__head{display:flex;justify-content:space-between;align-items:center;padding:16px 20px;border-bottom:1px solid #eee}',
      '.pod-modal__head h3{margin:0;font:600 18px system-ui}',
      '.pod-modal__body{padding:20px}',
      ...
      '.wz-area{min-height:220px;padding:12px 14px;font:15px/1.6 Georgia,serif}',
  ...
  function openGallery() {
    ...
    var up = el('div', { class: 'pod-field' },
      '<label>Nahrať fotky</label><input type="file" id="pod-up" accept="image/*" multiple>');
  ...
  function openArticles() {
    ...
    var list = el('ul', { class: 'pod-list' });
    body.appendChild(newBtn); body.appendChild(el('div', { style: 'height:12px' })); body.appendChild(list);
    newBtn.addEventListener('click', function () { editArticle(null); });
    ...
    function load() {
      j(API + '/admin/articles').then(function (r) {
        ...
        (r.body || []).forEach(function (a) {
          ...
          edB.addEventListener('click', function () { editArticle(a.id); });
    ...
    function editArticle(id) {
      ...
      wrap.innerHTML =
        ...
        '<div style="display:flex;gap:8px"><button class="pod-btn" id="a-save">Uložiť</button><button class="pod-btn ghost" id="a-cancel">Zrušiť</button></div>';
